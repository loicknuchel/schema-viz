...
suite =
    describe "AlterTable"
        [ describe "parseAlterTable"
            [ stmCheck "primary key" "ALTER TABLE public.t2 ADD CONSTRAINT t2_id_pkey PRIMARY KEY (id);" parseAlterTable (\_ -> Ok (AddTableConstraint (Just "public") "t2" (ParsedPrimaryKey "t2_id_pkey" [ "id" ])))
            ...
            , stmCheck "unique" "ALTER TABLE p.t1 ADD CONSTRAINT name_unique UNIQUE (first_name, last_name);" parseAlterTable (\_ -> Ok (AddTableConstraint (Just "p") "t1" (ParsedUnique "name_unique" { columns = [ "first_name", "last_name" ], definition = "(first_name, last_name)" })))
            ...
            , stmCheck "without schema" "ALTER TABLE t2 ADD CONSTRAINT t2_id_pkey PRIMARY KEY (id);" parseAlterTable (\_ -> Ok (AddTableConstraint Nothing "t2" (ParsedPrimaryKey "t2_id_pkey" [ "id" ])))
            , stmCheck "with only" "ALTER TABLE ONLY public.t2 ADD CONSTRAINT t2_id_pkey PRIMARY KEY (id);" parseAlterTable (\_ -> Ok (AddTableConstraint (Just "public") "t2" (ParsedPrimaryKey "t2_id_pkey" [ "id" ])))
